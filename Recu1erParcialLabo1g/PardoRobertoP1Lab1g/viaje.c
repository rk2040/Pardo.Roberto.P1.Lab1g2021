    ...
    for(int i=0; i<tamMicro; i++){
        if(unViaje.idMicro == micros[i].id){
            ...
            if(cargarDescripcionEmpresa(empresas, tamEmpresa, micros[indice].idEmpresa, descEmpresa) == 1 &&
               ...
               cargarNombreChofer(choferes, tamChofer, micros[indice].idChofer, nombreChofer) == 1 )
            {

                printf(" %2d       %-20s %-20s%-20s $%10.2f     %02d/%02d/%d    %-20s\n",
                       unViaje.id,
                       descEmpresa,
                       descTipo,
                       descDestino,
...
int mostrarViajes(eViaje viajes[], int tamViaje, eMicro micros[], int tamMicro, eEmpresa empresas[], int tamEmpresa, eTipo tipos[], int tamTipo, eDestino destinos[], int tamDestino, eChofer choferes[], int tamChofer){
    ...
    if(viajes != NULL && tamViaje > 0){
        ...
        for(int i=0; i<tamViaje; i++){
            if(!viajes[i].isEmpty){
                mostrarViaje(viajes[i], micros, tamMicro, empresas, tamEmpresa, tipos, tamTipo, destinos, tamDestino, choferes, tamChofer);
                flag = 0;
            }
        }
        if(flag){
